...
const combinedFiles = [...filesFromDeploymentsDir, ...filesFromDeploymentsContract];
...
> => {
  ...
  combinedFiles.forEach((file) => {
    const dir = file.startsWith('cairoscripts_') ? deploymentsDir : deployments_Contract;
    const filePath = path.join(dir, file);
    if (path.extname(file) === ".json" && (file.endsWith("_state.json") || file.endsWith("cairoscripts.starknet_artifacts.json"))) {
      try {
      ...
      if (jsonData.transactions && jsonData.transactions.transactions) {
        for (const tx of Object.values(jsonData.transactions.transactions)) {
          if (tx.name === 'deploy' && tx.output && tx.output.contract_address) {
      ...
      if (Array.isArray(jsonData.contracts) && jsonData.contracts.length > 0) {
      ...
      Object.entries(combinedJsonArray).forEach(([contractName, data]) => {
        if(data.contract_name !== '' && data.address !== '' && data.class_hash !== ''){
          try {
