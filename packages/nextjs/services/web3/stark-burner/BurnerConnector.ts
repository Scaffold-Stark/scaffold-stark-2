...
export class BurnerConnector extends InjectedConnector {
  ...
  async account(): Promise<AccountInterface> {
    const address = "0x64b48806902a367c8598f4f95c305e8c1a1acba5f082d294a43793113115691";
    const privateKey = "0x71d7bb07b9a64f6f78ac4c816aff4da9"
    return Promise.resolve(new Account(new RpcProvider({nodeUrl: this.chain.rpcUrls.public.http[0], chainId: starknetChainId(this.chain.id)}), address, privateKey));
  }
  ...
  async connect(): Promise<Required<ConnectorData>> {
    return Promise.resolve({account: (await this.account()).address, chainId: this.chain.id});
  }
