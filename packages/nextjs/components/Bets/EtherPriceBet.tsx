...
type EtherPriceBetProps = { etherPriceData: any; isLoading: boolean };

function EtherPriceBet({ etherPriceData, isLoading }: EtherPriceBetProps) {
  const { address, status, chainId, ...props } = useAccount();
  const [amountEth, setAmountEth] = React.useState(0);
  const { data } = useDeployedContractInfo("EtherPrice");
  const { writeAsync: writeAsyncYes } = useScaffoldMultiWriteContract({
    calls: [
      ...
      createContractCall("EtherPrice", "vote_yes", [amountEth * 10 ** 18]),
    ],
  });
  const { writeAsync: writeAsyncNo } = useScaffoldMultiWriteContract({
    calls: [
      ...
      createContractCall("EtherPrice", "vote_no", [amountEth * 10 ** 18]),
    ],
  });
    ...
    <div>
      ...
      >
        <div className="flex flex-col items-center justify-center p-4 rounded-lg">
          ...
          <p className="text-primary text-2xl font-bold">
            {parseFloat(formatUnits(etherPriceData?.total_amount)).toFixed(4)}
            &nbsp;ETH
          </p>
        </div>
      </ShineBorder>
