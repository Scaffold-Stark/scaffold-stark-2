...
const ChallengeCard: React.FC<ChallengeCardProps> = ({ challenge, title, imageUrl, buttonText, onButtonClick,description,border=true, end=false  }) => {
    return (
        <div className={`flex max-w-[1280px] w-full  ${border ? 'border-b-2 border-base-300' : ''}`}>
            <div className="flex">
                <div className={`w-[34px] ${end? "h-[50%] self-end":"flex"}  border-l-[5px] border-base-300`}>
                    <div
                        className="w-[30px] h-[30px] rounded-full border-base-300 border-[5px] self-center traslate-circule bg-base-100"></div>
                </div>
            </div>
