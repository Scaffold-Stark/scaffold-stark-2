...
const ChallengeCard: React.FC<ChallengeCardProps> = ({ challenge, title, imageUrl, buttonText, onButtonClick,description,border=true, end=false  }) => {
    return (
        ...
        <div className={`flex max-w-[1280px] w-full lg:text-[12px] sm:text-[12px] lg:text-sm ${border ? 'border-b-2 border-base-300 sm:border-b-[1px]' : ''}`}>
            <div className="flex">
                <div className={`w-[34px] ${end? "h-[50%] self-end":"flex"}  border-l-[5px] border-base-300 lg:border-l-[3px] sm:border-l-[3px]`}>
                    <div
                        className="w-[30px] h-[30px] rounded-full border-base-300 border-[5px] self-center traslate-circule bg-base-100 sm:w-[15px] sm:h-[15px] lg:border-[3px] lg:h-[15px] lg:w-[15px] sm:border-[3px] "></div>
                </div>
            </div>
