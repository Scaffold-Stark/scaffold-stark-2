...
export async function GET(request: NextRequest) {
  ...
  const serializedBets = JSON.stringify(
    bets.map((bet) => ({
      ...bet,
      ...
      outcome_yes_bought_amount_with_yield:
        bet.outcome_yes_bought_amount_with_yield.toString(),
      outcome_no_bought_amount: bet.outcome_no_bought_amount.toString(),
      outcome_no_bought_amount_with_yield:
        bet.outcome_no_bought_amount_with_yield.toString(),
    })),
  );
